...
@section('title', 'Ubah Data Kegiatan Rapat Koordinasi')
...
@section('content')
	<div class="right_col" role="main">
		...
		<div class="row">
			...
			<div class="x_panel">
				<div class="x_title">
					<h2>Form Ubah Data Kegiatan Rapat Koordinasi Direktorat Advokasi</h2>
				...
				<div class="x_content">
						...
						@endif
					<form action="{{URL('/pencegahan/dir_advokasi/update_pendataan_koordinasi')}}" class="form-horizontal" id="frm_add" method="post" enctype="multipart/form-data" autocomplete="on">
	       			 {{ csrf_field() }}
    				...
    				<div class="form-body">
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								<input value="{{$data_detail['data']['no_sprint']}}" required id="no_sprint" name="no_sprint" type="text" class="form-control">
							</div>
						</div>
				        ...
				        <div class="form-group">
				            ...
				            <div class='col-md-6 col-sm-6 col-xs-12 input-group date tanggal'>
				              ...
				              <input type='text' name="tgl_pelaksanaan" required value="{{ \Carbon\Carbon::parse($data_detail['data']['tgl_pelaksanaan'] )->format('d/m/Y') }}" class="form-control" />
				              @else
				              <input type="text" name="tgl_pelaksanaan" required value="" class="form-control" />
				              @endif
							...
							<div class="col-md-6">
									<div class="mt-repeater">
											<div data-repeater-list="meta_panitia">
												@if($data_detail['data']['meta_panitia'])
					               @foreach(json_decode($data_detail['data']['meta_panitia'],true) as $r1 )
													<div data-repeater-item="" class="mt-repeater-item">
															<div class="row mt-repeater-row">
																<div class="col-md-10">
																	<label class="control-label">Nama Panitia</label>
																		<select class="form-control selectPanitia" name="meta_panitia[{{$r1}}][panitia_monev]" required>
																			<option value="">-- Pilih Panitia --</option>
																			@if ($panitia['code'] == 200)
																				@foreach($panitia['data']['pegawai'] as $w)
																					<option value="{{$w['nama']}}" {{($w['nama'] == $r1) ? 'selected="selected"':""}} >{{$w['nama']}}</option>
																				@endforeach
																			@endif
																		</select>
																</div>
				        ...
				        <div class="form-group">
		                  ...
		                  <div class="col-md-4 col-sm-4 col-xs-12 m-t-10">
		                  	<div class="row">
			                  	<?php
			                  		$meta_sasaran = $data_detail['data']['meta_sasaran'];
			                  		$kode_sasaran = [];
			                  		if($meta_sasaran){
			                  			$arr_sasaran = json_decode($meta_sasaran,true);
			                  			if(count($arr_sasaran)>0){
			                  				for($i = 0 ; $i < count($arr_sasaran); $i++){
			                  					$kode_sasaran[] =  $arr_sasaran[$i];
			                  				}
			                  			}else{
			                  				$kode_sasaran = [];
			                  			}
			                  		...
			                  		@if(count($sasaran) > 0 )
			                  			@foreach($sasaran as $skey => $sval)
			                  				<label class=" col-md-9 col-sm-9 col-xs-12">
						                    	<input type="checkbox" {{(  in_array($skey, $kode_sasaran)  ? 'checked=checked' : '' )}} value="{{$skey}}" name="meta_sasaran[]">
						                      	<span>&nbsp;{{$sval}}</span>
						                    </label>
			                  			@endforeach
			                  		@endif
				        ...
				        <div class="form-group">
				            <label for="instansi" class="col-md-3 control-label">Instansi Peserta</label>
				            <div class="col-md-8">
				                <div class="mt-repeater">
					                    ...
					                    @else
					                    	<div data-repeater-item="" class="mt-repeater-item">
						                            <div class="row mt-repeater-row">
						                                <div class="col-md-4">
						                                    <label class="control-label">Nama Instansi</label>
						                                    <input name="group-c[0][list_nama_instansi]" value="" type="text" class="form-control"> </div>
						                                <div class="col-md-5">
						                                    <label class="control-label">Jumlah Peserta</label>
						                                    <input name="group-c[0][list_jumlah_peserta]" value="" type="text" class="form-control col-md-7 col-xs-12 numeric" onKeydown="numeric_only(event,this)"> </div>
						                                <div class="col-md-1">
						                                    <a href="javascript:;" data-repeater-delete="" class="btn btn-danger mt-repeater-delete">
						                                        <i class="fa fa-close"></i>
						                                    </a>
				            ...
				            <div class="col-md-5">
				                ...
				                <span class="help-block" style="color:white">
				                    @if (!empty($data_detail['data']['file_upload']))
				                        lihat file : <a style="color:yellow" href="{{\Storage::url('AdvokasiRakor/'.$data_detail['data']['file_upload'])}}">{{$data_detail['data']['file_upload']}}</a>
				                    @endif
				                </span>
				            </div>
				        </div>
