...
@section('title', 'Sekretariat Utama')
    ...
    <div class="right_col edit_settama" role="main">
        ...
        <div class="row">
            <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                    <div class="x_title">
                        <h2>Form Ubah Kegiatan Sekretariat Utama</h2>
                            <div class="clearfix"></div>
                    </div>
                    <div class="x_content">
                        <br />
                        ...
                        @endif
                        <form  method="POST" action="{{route('update_sekretariat_utama')}}" data-parsley-validate class="form-horizontal " enctype="multipart/form-data" id="multipart-form" onSubmit="validateForm(event,this)">
                        {{csrf_field()}}
                        <input type="hidden" value="{{$data->id_settama}}" name="id"/>
                            <div class="form-body">
                                                ...
                                                @if(count($rujukan))
                                                    @foreach($rujukan as $rkey => $rval )
                                                        <div class="group">
                                                            ...
                                                            <div class="mt-radio col-md-12 col-sm-12 col-xs-12 m-l-10 m-b-10  {{(isset($data) ? ( (trim($data->jns_rujukan) == $rkey) ? '' :'hide'):'hide')}} nomor">
                                                                <input type="text" name="no_rujukan" value="{{(isset($data) ? ( (trim($data->jns_rujukan) == $rkey) ? $data->no_rujukan :''):'')}}" class="form-control" placeholder="no rujukan" />
                                                            </div>
                                                        </div>
                                                    @endforeach
                                                @endif
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select name="pelaksana" class="form-control select2 list_pelaksana" onChange="getBagianPelaksana(this)">
                                            <option value="">-- Pilih Pelaksana--</option>
                                            @if(isset($pelaksana))
                                                @if(count($pelaksana))
                                                    @foreach($pelaksana as $pkey => $pval )
                                                       <option value="{{trim($pval->id_settama_lookup)}}" data-target="{{ trim($pval->id_settama_lookup)}}" {{( isset($data) ? (trim($pval->id_settama_lookup) == trim($data->pelaksana) ? 'selected=selected' : ''):'')}}> {{trim($pval->lookup_name)}} </option>
                                                    @endforeach
                                                @endif
                                            @endif
                                        </select>
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12">
                                        <select name="bagian" class="form-control pelaksana select2">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-md-6 col-sm-6 col-xs-12 ">
                                        <select name="jenis_kegiatan" class="form-control jenis_kegiatan select2">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-6 col-sm-6 col-xs-12 ">
                                    <div class="radio">
                                      <label class="mt-radio col-md-9 col-sm-9 col-xs-12"> <input disabled type="radio" {{(isset($data) ? ( trim($data->sumber_anggaran) == 'DIPA' ? 'checked=checked' :'') : '')}} value="DIPA" name="sumber_anggaran" id="anggaran1">
                          						<span>Dipa</span>
                          						</label>
                          						<label class="mt-radio col-md-9 col-sm-9 col-xs-12"> <input disabled type="radio" {{(isset($data) ? ( trim($data->sumber_anggaran) == 'NONDIPA' ? 'checked=checked' :'') : '')}} value="NONDIPA" name="sumber_anggaran" id="anggaran2">
                          						<span>Non Dipa</span>
                          						</label>
                                    </div>
                                </div>
                                      ...
                                      <span class="help-block white">
                                          @if (isset($data))
                                            @if($data->file_laporan)
                                              @if (File::exists($file_path.$data->file_laporan))
                            ...
                            <div class="form-actions fluid m-t-20">
                                <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-3">
                                    ...
                                    <a href="{{route('sekretariat_utama')}}" class="btn btn-primary" type="button">BATAL</a>
                                </div>
                            </div>
                        </form>
                    </div>
