...
@if(isset($data))
	@if(isset($data->bidang))
		...
		@if(count($b->kinerja) > 0 )
			@php $j = 1; @endphp
			@for($i = 0 ; $i < count($b->kinerja) ; $i++) 
				@php $k = $b->kinerja[$i]; @endphp
					...
					<form class="form-horizontal form-ptl text-left" >
						...
						<button data-toggle="collapse" data-target="#form_kinerja_{{$i}}" class="btn btn-primary btn-open collapsed" type="button" ></button>
						<div class="collapse" id="form_kinerja_{{$i}}">
							<div class="form-body">
								<div class="form-group">
