...
@if(isset($data))
	@if(isset($data->bidang))
		...
		@if(count($b->keuangan) > 0 )
			@php $j = 1; @endphp
			@for($i = 0 ; $i < count($b->keuangan) ; $i++) 
				@php $k = $b->keuangan[$i]; @endphp
					...
					<form class="form-horizontal form-ptl text-left" >
						...
						<button data-toggle="collapse" data-target="#form_keuangan_{{$i}}" class="btn btn-primary btn-open collapsed" type="button" ></button>
						<div class="collapse" id="form_keuangan_{{$i}}">
							<div class="form-body">
								<div class="form-group">
					...
					<tbody>
						...
						@else
							<tr>
								<td colspan="6">
									<div>
										Data Bidang Keuangan Belum Tersedia
									</div>
								</td>
							</tr>
						@endif
	...
	@endif
@endif
<input type="hidden" name="tipe" value="keuangan"/>
</div>