...
@if(isset($data))
	@if(isset($data->bidang))
		...
		@if(count($b->sarana) > 0 )
			@php $j = 1; @endphp
			@for($i = 0 ; $i < count($b->sarana) ; $i++) 
				@php $k = $b->sarana[$i]; @endphp
					...
					<form class="form-horizontal form-ptl text-left" >
						...
						<button data-toggle="collapse" data-target="#form_sarana_{{$i}}" class="btn btn-primary btn-open collapsed" type="button" ></button>
						<div class="collapse" id="form_sarana_{{$i}}">
							<div class="form-body">
								<div class="form-group">
					...
					<tbody>
						...
						@else
							<tr>
								<td colspan="6">
									<div>
										Data Bidang Sarana Prasarana Belum Tersedia
									</div>
								</td>
							</tr>
						@endif
	...
	@endif
@endif
<input type="hidden" name="tipe" value="sarana"/>
</div>