				...
				<div class="x_panel">
					<div class="x_title">
						...
						<ul class="nav navbar-right panel_toolbox">
							{{-- <li class=""> --}}
              				<li class="" @php if(!in_array(27, Session::get("cancreate")))  echo 'style="display:none;"'; @endphp>
							...
							{{-- <li class=""> --}}
              				<li class="" @php if(!in_array(27, Session::get("cancreate")))  echo 'style="display:none;"'; @endphp>
								<a href="{{url('pemberantasan/dir_interdiksi/add_pendataan_intdpo')}}" class="btn btn-lg btn-round btn-primary">
					...
					<div class="x_content ">
						<br/>
				            ...
				            @endif
				            @include('_templateFilter.interdiksi_dpo_filter')
						...
						<table id="datatable-responsive" class="table table-striped dt-responsive nowrap text-left" cellspacing="0" width="100%">
							...
							@if(count($data_kasus))
								...
								<tr>
									...
									{{--<td>
										<a href="{{url('pemberantasan/dir_interdiksi/edit_pendataan_intdpo/'.$d['eventID'])}}">
									...
									<td>
										<a @php if(!in_array(27, Session::get("canedit")))  echo 'style="display:none;"'; @endphp href="{{url('pemberantasan/dir_interdiksi/edit_pendataan_intdpo/'.$d['eventID'])}}">
										<i class="fa fa-pencil"></i></a>
										<button @php if(!in_array(27, Session::get("candelete")))  echo 'style="display:none;"'; @endphp data-url='kasus' type="button" class="btn btn-primary button-action" onClick="delete_form(event,this)" data-target="{{$d['eventID']}}" ><i class="fa fa-trash"></i></button>
									</td>
								</tr>
											@php $i = $i+1; @endphp
											@endforeach
