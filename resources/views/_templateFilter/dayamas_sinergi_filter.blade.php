          ...
          <div class="col-md-6 col-sm-6 col-xs-12 jenis_kegiatan {{(isset($filter['tipe']) ? (($filter['tipe'] == 'jenis_kegiatan') ? '' : 'hide') :'hide')}} div-wrap">
            <label for="tipe" class="control-label"> Jenis Kegiatan</label>
             <select class="form-control select2 " name="jenis_kegiatan">
                ...
                @if(count($bentuk_kegiatan) > 0 )
                    @foreach($bentuk_kegiatan as $bkode=>$kbvalue)
                      <option value="{{$bkode}}" {{( isset($filter['jenis_kegiatan']) ? ( ( $filter['jenis_kegiatan'] ==$bkode)  ? 'selected=selected' : '') : '')}}>{{$kbvalue}}</option>
                    @endforeach
                @endif
            </select>
          </div>
      ...
      @if(isset($filter))
        ...
        @if( isset($s['tipe']))
          ...
          @elseif($s['tipe'] == 'jenis_kegiatan')
            {{( isset($bentuk_kegiatan[$filter['jenis_kegiatan']]) ? $bentuk_kegiatan[$filter['jenis_kegiatan']].',' : '')}}
