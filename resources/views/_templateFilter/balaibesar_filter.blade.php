    ...
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 keyword {{(isset($filter['tipe']) ? (($filter['tipe'] == 'nama_kegiatan') ? '' : 'hide') :'')}} div-wrap">
            <label for="tipe" class="control-label">Kata Kunci</label>
            <input class="form-control" name="kata_kunci" value="{{isset($filter) ? (isset($filter['nama_kegiatan'])?$filter['nama_kegiatan']:'') :''}}" {{isset($filter['nama_kegiatan']) ? ($filter['nama_kegiatan'] ? '':'disabled=disabled') :'disabled=disabled'}}  />
          </div>
            ...
            <label for="tipe" class="control-label"> Jenis Kegiatan</label>
             <select class="form-control select2 " name="jenis_kegiatan">
                ...
                @if(count($jenis_kegiatan) > 0 )
                  @foreach($jenis_kegiatan as $jkey => $jval)
                    <option value="{{$jkey}}" {{( isset($filter['jenis_kegiatan']) ? ( ($filter['jenis_kegiatan'] == $jkey) ? 'selected=selected' :'' ) :''  )}}>{{$jval}}</option>
                  @endforeach
                @endif
            </select>
          </div>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 instansi {{(isset($filter['tipe']) ? (($filter['tipe'] == 'instansi') ? '' : 'hide') :'hide')}} div-wrap">
            <label for="tipe" class="control-label">Instansi</label>
            <select class="form-control select2"  name="instansi">
              <option>-- Pilih Instansi</option>
              @if(count($instansi) > 0 )
                  @foreach($instansi as $ikey => $ival)
                    <option value="{{$ikey}}" {{( isset($filter['instansi']) ? ( ($filter['instansi'] == $ikey) ? 'selected=selected' :'' ) :''  )}}>{{$ival}}</option>
                  @endforeach
                @endif
            </select>
          </div>
      ...
      @if(isset($filter))
        ...
        @if( isset($s['tipe']))
          ...
          @elseif($s['tipe'] == 'jenis_kegiatan')
            {{( isset($jenis_kegiatan[$filter['jenis_kegiatan']]) ? $jenis_kegiatan[$filter['jenis_kegiatan']].',' : '')}}
          @elseif($s['tipe'] == 'instansi')
           {{( isset($instansi[$filter['instansi']]) ? $instansi[$filter['instansi']] :  $filter['instansi'])}} ,
          @endif
        @endif
