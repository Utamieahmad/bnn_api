    ...
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 keyword {{( isset($filter['tipe']) ?  ( (($filter['tipe'] == 'pengirim') || ($filter['tipe'] == 'penerima')|| ($filter['tipe'] == 'subyek')|| ($filter['tipe'] == 'konten'))  ? '' : 'hide')  :'' )}} div-wrap">
            <label for="tipe" class="control-label">Kata Kunci</label>
            <?php
              $kata_kunci = "";
              $tipe_keyword = ['pengirim','penerima','subyek','konten'];
          ...
          <div class="tgl_terima {{(isset($filter['tipe']) ? (($filter['tipe'] == 'tgl_terima') ? '' : 'hide') :'hide')}} div-wrap">
          ...
          <div class="tgl_input {{(isset($filter['tipe']) ? (($filter['tipe'] == 'tgl_input') ? '' : 'hide') :'hide')}} div-wrap">
            <div class="col-md-6 col-sm-6 col-xs-12 m-t-10">
              <label for="tipe" class="control-label">Mulai</label>
              <div class='input-group date date_start row'>
                  <input type='text' name="tgl_input_from" class="form-control" value="{{isset($filter) ? (isset($filter['tgl_input_from']) ? ($filter['tgl_input_from']? $filter['tgl_input_from']  :'' ): '') :''}}"/>
                  <span class="input-group-addon">
                  <span class="glyphicon glyphicon-calendar"></span>
                  </span>
              </div>
            ...
            <div class="col-md-6 col-sm-6 col-xs-12 m-t-10">
              <label for="tipe" class="control-label">Selesai</label>
              <div class='input-group date date_end row'>
                  <input type='text' name="tgl_input_to" class="form-control" value="{{isset($filter) ? (isset($filter['tgl_input_to'])? ($filter['tgl_input_to'] ? $filter['tgl_input_to'] : ''):'') :''}}"/>
                  <span class="input-group-addon">
                  <span class="glyphicon glyphicon-calendar"></span>
                  </span>
              </div>
  ...
  <div class="m-b-20">
    <div class="m-b-20">
      ...
      @if(isset($filter))
        ...
        @if( isset($s['tipe']))
          @if( $s['tipe'] == 'tgl_terima')
            {{isset($s['tgl_from']) ? $s['tgl_from'] : ''}}
            {{isset($s['tgl_from']) && isset($s['tgl_to'])  ? '-' : ''}}
            {{isset($s['tgl_to']) ?  $s['tgl_to'] .', ' : ', '}}
          @elseif( $s['tipe'] == 'tgl_input')
            {{isset($s['tgl_input_from']) ? $s['tgl_input_from'] : ''}}
            {{isset($s['tgl_input_from']) && isset($s['tgl_input_to'])  ? '-' : ''}}
            {{isset($s['tgl_input_to']) ?  $s['tgl_input_to'] .', ' : ', '}}
          @else
            {{ $filter[$filter['tipe']] }} ,
          @endif
        @endif
