            ...
            <select class="form-control select2"  name="pelaksana" >
              ...
              @if(isset($instansi) > 0 )
                @foreach($instansi as $i => $ival)
                  <option value="{{$ival['nm_instansi']}}" {{( isset($filter['pelaksana']) ? ( ( $filter['pelaksana'] == $ival['nm_instansi'] )  ? 'selected=selected' : '') : '')}}>{{$ival['nm_instansi']}}</option>
                @endforeach
              @endif
            </select>
          </div>
    ...
    <div class="m-b-20">
      ...
      @if(isset($filter))
        ...
        @if( isset($s['tipe']))
          ...
          @else
            {{( isset($filter[$filter['tipe']] ) ?  $filter[$filter['tipe']] : $filter['tipe']  )}} ,
          @endif
        @endif
