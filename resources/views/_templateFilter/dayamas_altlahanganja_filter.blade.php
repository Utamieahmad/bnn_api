    ...
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 keyword {{(isset($filter['tipe']) ? (($filter['tipe'] == 'pelaksana') ? '' : 'hide') :'')}} div-wrap">
            <label for="tipe" class="control-label">Kata Kunci</label>
            <input class="form-control" name="kata_kunci" value="{{isset($filter) ? (isset($filter['pelaksana'])?$filter['pelaksana']:'') :''}}" {{isset($filter['pelaksana']) ? ($filter['pelaksana'] ? '':'disabled=disabled') :'disabled=disabled'}}  />
          </div>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 komoditi {{(isset($filter['tipe']) ? (($filter['tipe'] == 'komoditi') ? '' : 'hide') :'hide')}} div-wrap">
            <label for="tipe" class="control-label">Penyelenggara</label>
            <select class="form-control select2"  name="komoditi">
              <option>-- Pilih Komoditi</option>
                @if(count($lahan_komoditi) > 0)
                    @foreach($lahan_komoditi as $ckey=>$cvalue)
                        <option value="{{$cvalue['nama_komoditi']}}"{{(isset($filter['komoditi']) ? (($filter['komoditi']== $cvalue['nama_komoditi']) ? 'selected=selected' : '' ) : '' )}}>{{$cvalue['nama_komoditi']}} </option>
                    @endforeach
                @endif
            </select>
          </div>
    ...
    <div class="m-b-20">
      ...
      @if(isset($filter))
        ...
        @if( isset($s['tipe']))
          ...
          @elseif($s['tipe'] == 'komoditi')
            {{( isset($lahan_komoditi[$filter['komoditi']]) ? $lahan_komoditi[$filter['komoditi']] : $filter['komoditi'] ) }},
