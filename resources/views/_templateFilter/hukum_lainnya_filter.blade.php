      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        <?php
          $key = [
            ...
            'bagian' => 'Bagian',
            'no_sprint_kepala' => 'Nomor Surat Perintah BNN',
            'periode' => 'Periode',
            'tema' => 'Tema',
            'status' => 'Status',
          ];
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12">
            ...
            <select class="form-control select2"  name="tipe" onchange="formFilter(this)">
              ...
              <option value="no_sprint_kepala" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'no_sprint_kepala') ? 'selected=selected' : '') :'')}}>Nomor Surat Perintah BNN</option>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 keyword {{(isset($filter['tipe']) ? ((($filter['tipe'] == 'periode') || ($filter['tipe'] == 'jenis_kegiatan') || ($filter['tipe'] == 'status') || ($filter['tipe'] == 'bagian')) ? 'hide' : '') :'')}} div-wrap">
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 jenis_kegiatan {{isset($filter['tipe']) ? ( $filter['tipe'] == 'jenis_kegiatan' ? '':'hide'): 'hide'}} div-wrap" >
              ...
              <select class="form-control select2" name="jenis_kegiatan">
                <option value="Sosialisasi" {{isset($filter)? (isset($filter['jenis_kegiatan']) ? ($filter['jenis_kegiatan'] == 'Sosialisasi' ? 'selected=selected' : '') :''):''}} >Sosialisasi</option>
                <option value="Monitoring_Evaluasi" {{isset($filter)? (isset($filter['jenis_kegiatan']) ? ($filter['jenis_kegiatan'] == 'Monitoring_Evaluasi' ? 'selected=selected' : '') :''):''}} >Monitoring dan Evaluasi</option>
                <option value="Rapat_Konsinyering" {{isset($filter)? (isset($filter['jenis_kegiatan']) ? ($filter['jenis_kegiatan'] == 'Rapat_Konsinyering' ? 'selected=selected' : '') :''):''}} >Rapat Konsinyering</option>
                <option value="Rapat_Antar_Kementrian" {{isset($filter)? (isset($filter['jenis_kegiatan']) ? ($filter['jenis_kegiatan'] == 'Rapat_Antar_Kementrian' ? 'selected=selected' : '') :''):''}} >Rapat Antar Kementrian</option>
              </select>
          </div>
        ...
        @if( isset($s['tipe']))
         ...
         @elseif($s['tipe'] == 'tema')
          ...
          @elseif($s['tipe'] == 'jenis_kegiatan')
            @if (isset($s['jenis_kegiatan']))
              @if($s['jenis_kegiatan'] == 'Sosialisasi')
                {{ 'Nota Sosialisasi' .' , ' }}
              @elseif($s['jenis_kegiatan'] == 'Monitoring_Evaluasi')
                {{ 'Monitoring dan Evaluasi' .' , ' }}
              @elseif($s['jenis_kegiatan'] == 'Rapat_Konsinyering')
                {{ 'Rapat Konsinyering'. ' , ' }}
              @elseif($s['jenis_kegiatan'] == 'Rapat_Antar_Kementrian')
