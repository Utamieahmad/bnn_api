  ...
  <form class="form-group filter" action="{{isset($route) ? route($route) : ''}}" method="post">
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        <?php
          $key = [
            'jenistahanan' => 'Jenis Tahanan',
            'nomor_kasus' => 'Nomor Kasus',
            'periode' => 'Tanggal Tahanan',
            // 'alamatdomisili' => 'Alamat Tahanan',
            'status' => 'Status',
          ];
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 keyword {{(isset($filter['tipe']) ? ((($filter['tipe'] == 'nomor_kasus' ) || ($filter['tipe'] == 'alamatdomisili') )  ? '' : 'hide') :'')}} div-wrap">
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 jenistahanan {{(isset($filter['tipe']) ? ((($filter['tipe'] == 'jenistahanan' )  )  ? '' : 'hide') :'hide')}} div-wrap">
              <label for="tipe" class="control-label">Tipe</label>

              <select class="form-control select2"  name="jenistahanan">
                <option>-- Pilih Jenis Tahanan -- </option>
                <option value="TAHANAN_BNN" {{isset($filter) ? ( isset($filter['jenistahanan'])? ($filter['jenistahanan'] == 'TAHANAN_BNN' ? 'selected=selected':''): '' ): ''}}> Tahanan BNN</option>
                <option value="TAHANAN_TITIPAN" {{isset($filter) ? ( isset($filter['jenistahanan'])? ($filter['jenistahanan'] == 'TAHANAN_TITIPAN' ? 'selected=selected':''): '' ): ''}}> Tahanan Titipan Instansi Lain</option>

              </select>
            </div>

             <!-- <div class="col-md-6 col-sm-6 col-xs-12 BrgBukti {{(isset($filter['tipe']) ? ((($filter['tipe'] == 'BrgBukti' )  )  ? '' : 'hide') :'hide')}} div-wrap">
              <label for="tipe" class="control-label">Tipe</label>

              <select class="form-control select2"  name="BrgBukti">
                <option> Jenis Barang Bukti  </option>
                @if(count($brgbukti))
                @foreach($brgbukti as $j => $jvalue)
                  <option disabled="disabled"> <strong> {{$j}}</strong> </option>
                  @if(count($jvalue))
                    @foreach($jvalue as $jkey => $jname)
                      <option value="{{$jkey}}" {{isset($filter) ? (isset($filter['nm_brgbukti'])? ( ($filter['nm_brgbukti'] == $jkey) ? 'selected=selected' : ''):'') :''}}> &nbsp;&nbsp;&nbsp; {{$jname}}</option>
                    @endforeach
                  @endif
                @endforeach
              @endif
    ...
    <div class="m-b-20">
      ...
      @endphp
        ...
        @if( isset($s['tipe']))
          ...
          @elseif($s['tipe'] == 'jenistahanan')
            {{( isset($s['jenistahanan'])  ?( ($s['jenistahanan'] == 'TAHANAN_TITIPAN' ) ? 'Tahanan Titipan Instansi Lain' .' ,' : 'Tahanan BNN'.' ,') :  '' )}}
