    ...
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 keyword {{( isset($filter['tipe']) ?  ( (($filter['tipe'] == 'pelapor') || ($filter['tipe'] == 'agen'))  ? '' : 'hide')  :'' )}} div-wrap">
            <label for="tipe" class="control-label">Kata Kunci</label>
            <?php
              $kata_kunci = "";
              $tipe_keyword = ['pelapor','agen'];
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 media_contact {{(isset($filter['tipe']) ? (($filter['tipe'] == 'media_contact') ? '' : 'hide') :'hide')}} div-wrap">
            <label for="tipe" class="control-label"> Media</label>
             <select class="form-control select2 " name="media_contact">
                <option>-- Pilih Media --</option>
                @if(isset($media))
                  @if(count($media) > 0)
                    @foreach($media as $kkey => $kval)
                      <option value="{{$kkey}}" {{ ( isset($filter['media_contact']) ? (($filter['media_contact'] == $kkey ) ? 'selected=selected' : '') : '')}}> {{$kval}} </option>
                    @endforeach
                  @endif
                @endif
            </select>
      ...
      @if(isset($filter))
        ...
        @if( isset($s['tipe']))
          ...
          @elseif($s['tipe'] == 'pelapor')
            {{ $filter['pelapor'] }} ,
          @elseif($s['tipe'] == 'media_contact')
            {{ isset( $media[$filter['media_contact']]) ? $media[$filter['media_contact']] : $filter['media_contact'] }} ,
          @elseif($s['tipe'] == 'agen')
            {{ $filter['agen'] }} ,
         @endif
        @endif
