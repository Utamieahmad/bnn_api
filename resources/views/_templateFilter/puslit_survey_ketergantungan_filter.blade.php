    ...
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 kategori {{(isset($filter['tipe']) ? (($filter['tipe'] == 'kategori') ? '' : 'hide') :'hide')}} div-wrap">
            <label for="tipe" class="control-label"> Status</label>
             <select class="form-control select2 " name="kategori">
                <option>-- Pilih Kategori --</option>
                @if(count($kategori) > 0 )
                  @foreach($kategori as $kat => $kat_val)
                    <option value="{{$kat}}" {{( isset($filter['kategori']) ? (($filter['kategori'] == $kat )? 'selected=selected' : '') : ''  )}}> {{$kat_val}}</option>
                  @endforeach
                @endif
            </select>
          </div>
                ...
                @if(isset($jenisBrgBuktiNarkotika))
                  @foreach($jenisBrgBuktiNarkotika as $keyGroup => $jenis )
                    <optgroup label="{{$keyGroup}}">
                      @foreach($jenis as $n => $nval)
                      <option value="{{preg_replace('/\s+/', '', $n)}}" {{( isset($filter['jns_narkoba']) ? (($filter['jns_narkoba'] == $n) ? 'selected=selected' : '') : '') }}>{{$nval}}</option>
                      @endforeach
                    </optgroup>
                  @endforeach
                @endif
      ...
      @if(isset($filter))
        ...
        @if( isset($s['tipe']))
          ...
          @elseif($s['tipe'] == 'jns_narkoba')
            ...
            @endif
          @elseif($s['tipe'] == 'kategori')
            @if($s['kategori'])
              {{ ( isset($kategori[$s['kategori']]) ? $kategori[$s['kategori']]: $s['kategori'] ) }} ,
            @endif
         @endif
        @endif
