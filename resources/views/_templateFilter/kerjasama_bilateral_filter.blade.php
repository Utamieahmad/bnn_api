  ...
  <form class="form-group filter" action="{{isset($route) ? route($route) : ''}}" method="post">
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        <?php
          $key = [
            'kodejeniskerjasama' => 'Jenis Kerja Sama',
            'tgl_pelaksana' => 'Tanggal Pelaksanaan',
            'tempatpelaksana' => 'Tempat Pelaksanaan',
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12">
            ...
            <select class="form-control select2"  name="tipe" onchange="formFilter(this)">
              ...
              <option value="kodejeniskerjasama" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'kodejeniskerjasama') ? 'selected=selected' : '') :'')}}>Jenis Kerja Sama</option>
              <option value="tgl_pelaksana" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'tgl_pelaksana') ? 'selected=selected' : '') :'')}}>Tanggal Pelaksanaan</option>
              <option value="tempatpelaksana" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'tempatpelaksana') ? 'selected=selected' : '') :'')}}>Tempat Pelaksanaan</option>
              <option value="materi" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'materi') ? 'selected=selected' : '') :'')}}>Nama Kegiatan</option>
              <option value="status" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'status') ? 'selected=selected' : '') :'')}}>Status Kelengkapan</option>
            </select>
          </div>
          <div class="col-md-6 col-sm-6 col-xs-12 keyword {{(isset($filter['tipe']) ? ((($filter['tipe'] == 'kodejeniskerjasama') || ($filter['tipe'] == 'tgl_pelaksana') || ($filter['tipe'] == 'status') ) ? 'hide' : '') :'')}} div-wrap">
          ...
          <div class="col-md-6 col-sm-6 col-xs-12 kodejeniskerjasama {{isset($filter['tipe']) ? ( $filter['tipe'] == 'kodejeniskerjasama' ? '':'hide'): 'hide'}} div-wrap" >
              <label for="tipe" class="control-label">Jenis Kerja Sama</label>

              <select class="form-control select2" name="kodejeniskerjasama">
                <option value="Sidang_Regional" {{isset($filter)? (isset($filter['kodejeniskerjasama']) ? ($filter['kodejeniskerjasama'] == 'Sidang_Regional' ? 'selected=selected' : '') :''):''}} >Sidang Regional</option>
                <option value="Sidang_Internasional"  {{isset($filter)? (isset($filter['kodejeniskerjasama']) ? ($filter['kodejeniskerjasama'] == 'Sidang_Internasional' ? 'selected=selected' : '') :''):''}} >Sidang Internasional</option>
                <option value="Pertemuan_Bilateral"  {{isset($filter)? (isset($filter['kodejeniskerjasama']) ? ($filter['kodejeniskerjasama'] == 'Pertemuan_Bilateral' ? 'selected=selected' : '') :''):''}} >Pertemuan Bilateral</option>
              </select>
          </div>
          ...
          <div class="tgl_pelaksana {{(isset($filter['tipe']) ? (($filter['tipe'] == 'tgl_pelaksana') ? '' : 'hide') :'hide')}} div-wrap">
  ...
  <div class="m-b-20">
    <div class="m-b-20">
      ...
      @if(isset($s))
        ...
        @if( isset($s['tipe']))
          @if( $s['tipe'] == 'tgl_pelaksana')
            {{isset($s['tgl_from']) ? $s['tgl_from'] : ''}}
            {{isset($s['tgl_from']) && isset($s['tgl_to'])  ? '-' : ''}}
            {{isset($s['tgl_to']) ?  $s['tgl_to'] .' , ' : ' , '}}
         @elseif($s['tipe'] == 'tempatpelaksana')
            {{  ( isset($tempatpelaksana[$s['keyword']])  ? $tempatpelaksana[$s['keyword']] .' , ' :  $s['keyword'] .', ' ) }}
          @elseif($s['tipe'] == 'status')
            {{( isset($s['status'])  ?( ($s['status'] == 'Y' ) ? 'Lengkap , ' : 'Belum Lengkap , ') :  '' )}}
          @elseif($s['tipe'] == 'kodejeniskerjasama')
            @if (isset($s['kodejeniskerjasama']))
              @if($s['kodejeniskerjasama'] == 'Sidang_Internasional')
                {{ 'Sidang Internasional' .' , ' }}
              @elseif($s['kodejeniskerjasama'] == 'Sidang_Regional')
                {{ 'Sidang Regional' .' , ' }}
              @elseif($s['kodejeniskerjasama'] == 'Pertemuan_Bilateral')
                {{ 'Pertemuan Bilateral'. ' , ' }}
              @endif
            @endif
          @else
