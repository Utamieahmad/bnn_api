  ...
  <form class="form-group filter" action="{{isset($route) ? route($route) : ''}}" method="post">
    <div class="row">
      ...
      <div class="col-md-6 col-sm-6 col-xs-12">
        ...
        <?php
          $key = [
            'no_permohonan' => 'No. Permohonan Peradilan',
        ...
        ?>
          ...
          <div class="col-md-6 col-sm-6 col-xs-12">
            ...
            <select class="form-control select2"  name="tipe" onchange="formFilter(this)">
              ...
              <option value="no_permohonan" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'no_permohonan') ? 'selected=selected' : '') :'')}}>No. Permohonan Peradilan</option>
              <option value="tgl_permohonan" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'nomor_perkara') ? 'selected=selected' : '') :'')}}>Tanggal Permohonan</option>
              <option value="permasalahan" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'permasalahan') ? 'selected=selected' : '') :'')}}>Permasalahan</option>
              <option value="nama_pemohon" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'nama_pemohon') ? 'selected=selected' : '') :'')}}>Nama Pemohon</option>
              <option value="status" {{(isset($filter['tipe']) ? (($filter['tipe'] == 'status') ? 'selected=selected' : '') :'')}}>Status Kelengkapan</option>
            </select>
          </div>
          <div class="col-md-6 col-sm-6 col-xs-12 keyword {{(isset($filter['tipe']) ? ((($filter['tipe'] == 'tgl_permohonan') || ($filter['tipe'] == 'status')) ? 'hide' : '') :'')}} div-wrap">
          ...
          <div class="tgl_permohonan {{(isset($filter['tipe']) ? (($filter['tipe'] == 'tgl_permohonan') ? '' : 'hide') :'hide')}} div-wrap">
    ...
    <div class="m-b-20">
      ...
      @if(isset($s))
        ...
        @if( isset($s['tipe']))
         ...
         @elseif($s['tipe'] == 'no_permohonan')
            {{  ( isset($no_permohonan[$s['keyword']])  ? $no_permohonan[$s['keyword']] .' , ' :  $s['keyword'] .', ' ) }}
         @elseif($s['tipe'] == 'permasalahan')
            {{  ( isset($permasalahan[$s['keyword']])  ? $permasalahan[$s['keyword']] .' , ' :  $s['keyword'] .' , ' ) }}
         @elseif($s['tipe'] == 'nama_pemohon')
            {{  ( isset($nama_pemohon[$s['keyword']])  ? $nama_pemohon[$s['keyword']] .' , ' :  $s['keyword'] .' , ' ) }}
