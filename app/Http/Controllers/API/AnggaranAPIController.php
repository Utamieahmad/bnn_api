...
class AnggaranAPIController extends Controller
{
    /* @author : Daniel Andi */

    public function index(Request $request)
    {
        $total_results = Anggaran::count();
        ...
        try {
            $data = Anggaran::orderBy('id', 'desc')->offset($offset)->limit($limit)->get();
            // $data = Anggaran::where('status', 1)->orderBy('id', 'desc')->offset($offset)->limit($limit)->get();;
            $paginate['page']       = $page;
