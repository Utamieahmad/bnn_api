...
use App\Models\Cegah\DiseminfoMediaPenyiaran;
use App\Models\Cegah\ViewDiseminfoMediaPenyiaran;
use App\Http\Controllers\Controller;
use App\Transformers\Json;

class DiseminfoMediaPenyiaranAPIController extends Controller
{
    /* @author : Daniel Andi */
    ...
    public function index(Request $request)
    {
        ...
        if ($request->jenis != '' ) {
            array_push($kondisi, array('jenis_media', 'ilike', '%'.$request->jenis.'%'));
        }
        ...
        // $total_results = ViewDiseminfoMediaPenyiaran::where($kondisi)->count();
        $qresults = ViewDiseminfoMediaPenyiaran::where($kondisi);
        ...
        try {
            ...
            // $data = DiseminfoMediaPenyiaran::where('status', 1)->orderBy('id', 'desc')->offset($offset)->limit($limit)->get();;
            $paginate['page']       = $page;
    ...
    {
        try {
            $data = DiseminfoMediaPenyiaran::where('id', $id)->first();
            // $data = DiseminfoMediaPenyiaran::where([['status', 1], ['tersangka_id', $id]])->first();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $data = DiseminfoMediaPenyiaran::create($request->except('api_token'));
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $data = DiseminfoMediaPenyiaran::findOrFail($id);
