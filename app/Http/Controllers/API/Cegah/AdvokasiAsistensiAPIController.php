...
use App\Models\Cegah\AdvokasiAsistensi;
use App\Models\Cegah\ViewAdvokasiAsistensi;
use App\Http\Controllers\Controller;
use App\Transformers\Json;

class AdvokasiAsistensiAPIController extends Controller
{
    /* @author : Daniel Andi */
    ...
    public function index(Request $request)
    {
      ...
      // $total_results = ViewAdvokasiAsistensi::where($kondisi)->count();
      $qresults = ViewAdvokasiAsistensi::where($kondisi);
      ...
      $total_results = $qresults->count();
        ...
        try {
            ...
            // $data = AdvokasiAsistensi::where('status', 1)->orderBy('id', 'desc')->offset($offset)->limit($limit)->get();;
            $paginate['page']       = $page;
    ...
    {
        try {
            $data = AdvokasiAsistensi::leftjoin('tr_wilayah', 'cegahadvokasi_asistensi.lokasi_kegiatan_idkabkota', '=', 'tr_wilayah.id_wilayah')->select('cegahadvokasi_asistensi.*', 'tr_wilayah.nm_wilayah AS lokasi_kegiatan_namakabkota')->where('id', $id)->first();
            // $data = AdvokasiAsistensi::where([['status', 1], ['tersangka_id', $id]])->first();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $data = AdvokasiAsistensi::create($request->except('api_token'));
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $data = AdvokasiAsistensi::findOrFail($id);
