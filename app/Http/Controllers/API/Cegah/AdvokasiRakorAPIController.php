...
use App\Models\Cegah\AdvokasiRakor;
use App\Models\Cegah\ViewAdvokasiRakor;
use App\Http\Controllers\Controller;
use App\Transformers\Json;

class AdvokasiRakorAPIController extends Controller
{
    /* @author : Daniel Andi */
    ...
    public function index(Request $request)
    {
        ...
        if ($request->sasaran != '' ) {
            array_push($kondisi, array('meta_sasaran', 'ilike', '%'.$request->sasaran.'%'));
        }
        if ($request->anggaran != '' ) {
        ...
        //$total_results = ViewAdvokasiRakor::where($kondisi)->count();
        $qresults = ViewAdvokasiRakor::where($kondisi);
        ...
        try {
            ...
            // $data = AdvokasiRakor::where('status', 1)->orderBy('id', 'desc')->offset($offset)->limit($limit)->get();;
            $paginate['page']       = $page;
    ...
    {
        try {
            $data = AdvokasiRakor::where('id', $id)->first();
    ...
    public function store(Request $request)
    {
        try {
            ...
            $data = AdvokasiRakor::create($request->except('api_token'));
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $data = AdvokasiRakor::findOrFail($id);
